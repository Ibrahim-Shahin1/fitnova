...
**Label direction (verified — important for honest reporting):** the official
`labels_shallow_depth.json` encodes the **positive class (label 1) as the deeper squat** — confirmed
**objectively by pose geometry** (MediaPipe hip-vs-knee on a per-label sample: label-1 crops sit
measurably deeper, hip nearer/below the knee — mean hip−knee −0.005 with 43% hip-below-knee, vs
label-0's −0.073 / 7%), and by inspecting the highest/lowest-scoring crops. This is counterintuitive
versus the dataset README's "1 = erroneous" wording, so we evaluate on the official labels exactly
as released — the same
protocol Parmar et al. use. The comparison is **robust to the convention**: positive-class F1
**0.8902**, flipped-class F1 **0.9050**, macro-F1 **0.8976**, ROC-AUC **0.9730** — all exceed the
published **0.8694**. The model is therefore a squat-**depth** classifier; in the app it is presented
in the physically-correct direction (high score = reached depth, low = too shallow).
...
test 0.8902) — small, no overfitting. The train→test gap is also small (train 0.971, test 0.890,
+0.081; baseline +0.107), confirming the image model generalizes (see `OVERFITTING_AUDIT.md`
Pillar 7). Confusion at the threshold: 223 TP / 13 FN / 42 FP / 262 TN
(precision 0.842, recall 0.945 on the positive class — label-1, the deeper squats).
...
| `shallow_squat_samples.png` | Sample crops by official label — positive (label-1, deeper) vs negative (label-0, shallower). *(Panel pos/neg wording follows the official label encoding; regenerate if a future caption flips it.)* |
...
- **Barbell Row was dropped** for compute/time — the milestone form-correction scope is
  Squat + OHP (video) + Shallow-Squat (image).
- **No live frontend:** a live, record-yourself form-correction frontend is not deployed, because
  the models do not transfer to home/phone-camera video (a known domain-shift limitation);
  instead the models are surfaced through a benchmark inspector that runs them on the dataset's own
  held-out test clips beside the expert ground truth. The deliverable is the trained/evaluated
  models + the paper comparison + this visualization pack (same stance as OHP).
- Like Squat/OHP, these results are on the **benchmark** (in-the-wild gym video). Every number here
  is from `backend/data/benchmark/shallow/results.pkl` or the local Fitness-AQA
...
#   backend/data/benchmark/shallow/results.pkl
