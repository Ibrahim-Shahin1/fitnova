...
| Error | Ours (official) | Parmar Ours-MD (paper, Table 4) | Our supervised baseline |
|-------|--------------------------|----------------------------------|-------------------------|
...
2-seed ensemble + early-stop close it. No validation-to-test overfitting in the reported model.
The train→test view (full audit, `OVERFITTING_AUDIT.md` Pillar 7) adds nuance: the deployed Elbows
train→test gap is +0.13, sharply reduced from the baseline's +0.53 (the SSL regularization effect),
with Knees +0.11; the baseline Elbows overfits on both axes (val→test +0.098 and train→test +0.53).
...
- **TTA not adopted** — evaluated on Squat and found to reverse on the test split;
...
- Like Squat, these results are on the **benchmark** (in-the-wild gym video, side-on). A
  live, record-yourself form-correction frontend is not deployed, because the models do not
  ...
  held-out test clips beside the expert ground truth.

## Reproduce

```
# results.pkl (per-clip scores + curves) is committed at
#   backend/data/benchmark/ohp/results.pkl
python backend/scripts/ohp_eval_viz.py        # SSL / PR / confusion / score-dist figures
python backend/scripts/ohp_finetune_curves.py backend/data/benchmark/ohp/results.pkl
