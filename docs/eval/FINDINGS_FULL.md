...
## EVAL-01 -- Master comparison table
...
## EVAL-02 -- MD+CVCSPC ensemble: evaluated, not-applicable
...
require either (a) re-running CVCSPC on the BackSquat labeled data -- out of scope per D1 (local
assembly from committed pickles, no re-runs) -- or (b) re-running MD on the Shallow-Squat static
...
EVAL-01 master table (their split protocols cannot be directly confirmed as identical to the
...
- Squat: macro 0.6304 vs Phase-3 supervised baseline 0.5429 -- **+0.087 SSL lift**.
...
(in-the-wild gym video, side-on). The form-correction frontend is cancelled; the models do not
transfer to home/phone camera -- the known in-the-wild/phone-camera domain-shift limit. The
defensible deliverable is this benchmark evaluation against the published numbers on identical
metrics and splits. Live/phone-camera domain adaptation is deferred to a future milestone.
...
- `.planning/phases/04-squat-motion-disentangling-ssl/figures/results.pkl` (Squat)
- `.planning/phases/06-overhead-press/figures/results.pkl` (OHP)
- `.planning/phases/07-image-based-errors-cvcspc/figures/results.pkl` (Shallow-Squat)
