...
| Error | Ours (official, Phase 4) | Parmar MD-SSL (paper) | Parmar Kinetics baseline |
|-------|--------------------------|------------------------|--------------------------|
...
generalization limit (the paper's own thesis). The **form-correction frontend
(live + upload-on-own-video) is therefore cancelled**; the defensible deliverable is the
benchmark evaluation above. Making it work on a user's own camera would require either
filming to match the training domain (side-on/landscape) or domain-adaptation fine-tuning.
