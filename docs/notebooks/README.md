...
- Recorded evaluation artifacts: per-clip/per-crop ensemble scores and per-epoch SSL +
  fine-tune curves, committed at `backend/data/benchmark/{squat,ohp,shallow}/results.pkl`.

## Reproduce

Each notebook is pre-executed (figures and tables render on open). To re-run one:
