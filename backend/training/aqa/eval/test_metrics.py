...
def test_f1_all_negative_predictions() -> None:
    # zero_division=0 path (RESEARCH §8): all-negative predictions → F1 == 0.0, no warning.
    y = _arr(1, 0, 1, 1)
    p = _arr(0, 0, 0, 0)
...
# ─────────────────────────────────────────────────────────────────────────────
# SQUAT-03 confusion matrix (2x2 shape regression for `labels=[0, 1]` defensive arg)
# ─────────────────────────────────────────────────────────────────────────────


def test_confusion_matrix_always_2x2() -> None:
    # All-positive labels AND predictions: sklearn would normally return 1x1
    # without explicit labels=[0, 1]. The regression test guards Task 2's defensive arg.
    y = _arr(1, 1, 1, 1)
    p = _arr(1, 1, 1, 1)
