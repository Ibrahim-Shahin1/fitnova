...
because Phase 3 trained with horizontal flip OFF, so flip is out-of-distribution
and must be validated, not assumed (RESEARCH §14 / CONTEXT D5: measure, don't guess).
...
ensemble aggregation in ``eval/ensemble.py`` — RESEARCH §14 aggregation order).
``select_tta_recipe`` is pure-numpy (uses ``eval/metrics.py``) and is importable +
testable without torch; ``tta_forward`` imports torch lazily at call time.
...
def tta_forward(
    ...
    for aug in recipe:
        if aug == "temporal_jitter":
            ...
            views.extend(_tta_five_crop(clip))  # center + 4 corners (inline, D9)
        else:
    ...
    # Mean LOGITS, not sigmoid — RESEARCH §14 aggregation order (per-seed TTA mean ->
...
def _tta_five_crop(clip: torch.Tensor) -> list:
    """Center + 4 corner crops of a ``[C,T,H,W]`` clip (RESEARCH §14 5-crop, inline per D9).
    ...
    crop=112 convention WITHOUT modifying transforms.py. NOTE (Plan 04): for full
    fidelity the 5-crop should run on the pre-crop frame; this self-contained upsample
    form is kept only if select_tta_recipe finds it improves val macro-F1.
    """
