...
from the **train** split regardless of which split this instance represents — the
`BCEWithLogitsLoss(pos_weight=...)` loss uses the train-derived weights for all loaders.

Label statistics: KIE = 14.29% positive (severe imbalance, w ≈ 6.0) and KFE = 68.33%
positive (mild reverse imbalance, w ≈ 0.47). Joint multi-label is justified by the
co-occurrence: 192/232 KIE+ clips are also KFE+.

`build_loaders(...)` returns `{"train": DataLoader, "val": DataLoader, "test": DataLoader}`.
`num_workers=0` is hardcoded — the bitwise-determinism guarantee requires single-worker
access.
"""
...
def _compute_pos_weight(train_records: list[splits.ClipRecord]) -> torch.Tensor:
    ...
    Returns:
        Float32 tensor of shape [2] — `[w_KIE, w_KFE]`. Used by
        `BCEWithLogitsLoss(pos_weight=...)`. Expected values: KIE ≈ 6.0,
        KFE ≈ 0.47 (tolerance ±0.05).
    """
...
class SquatKIEKFEDataset(Dataset):
    ...
    `restore_rng_state`, which is what makes the bitwise-resume assertion work
    ...
    The `seed` kwarg is reserved (may later bind it to a per-instance generator
    when num_workers > 0); it is currently a no-op — the trainer's
    ...
    def __init__(
        ...
        *,
        ...
        # represents. So val_dataset.pos_weight == train_dataset.pos_weight, and the loss
        # can be built once with weights computed a single time.
        if split == "train":
        ...
        # No per-instance generator (reserved for a future num_workers > 0 path).
        ...
        # across resume boundaries — load-bearing for the bitwise-resume assertion.
...
def build_loaders(
    ...
    **dataset_kwargs,
    ...
    Args:
        batch_size:     batch size for all three loaders (default 2).
        num_workers:    clamped to 0. A caller passing >0 gets a warning + the 0
                        override. This can later be lifted with a `worker_init_fn`
                        that re-seeds per-worker RNGs.
    ...
    """
    # num_workers = 0 is a bitwise-determinism precondition.
    if num_workers != 0:
        logger.warning(
            "num_workers=%d overridden to 0 (bitwise-determinism requires single-worker "
            "access; this may later be lifted with worker_init_fn)",
            num_workers,
        )
