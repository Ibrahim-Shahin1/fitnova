...
Wraps `torchvision.io.read_video` behind `decode_clip(path, indices)` so a future swap to TorchCodec (when/if Colab's torchvision passes 0.24 / `read_video` removal) is a one-line change. Window-bounded decode caps memory at the picked-indices span instead of reading the full clip, defending against OOM on long-tail Squat clips (up to 404 frames at width 480 × height up to 600 — ~350 MB uncompressed if decoded whole).
...
# torchvision >= 0.26 REMOVED `read_video` / `read_video_timestamps` as part of the
# TorchCodec migration (observed on tv 0.26.0+cu128). On those versions `decode_clip` +
# `count_frames` transparently fall back to a cv2 backend (opencv-python-headless, already
# a project dependency). Detected once at import so the per-call routing is cheap. The cv2
# path mirrors the serving twin `backend/services/clip_decode.decode_clip_cv2` (kept here so
# the training transforms layer stays self-contained — no training->services import).
...
def uniform_sample_indices(
    ...
    *,
    ...
    Returns:
        ...
        saturating clamp — no exception; no Squat clip is < 49 frames, so this is
        defensive only.
    """
...
def decode_clip(path: str, indices: torch.Tensor) -> torch.Tensor:
    ...
    Window-bounded decode: rather than decode the entire clip and fancy-index — which
    risks OOM on long Squat clips (up to 404 frames at 480×600×3 uint8 ≈ 350 MB raw) —
    we (1) cheaply probe `read_video_timestamps` for fps + total length, (2) compute
    `start_pts` and `end_pts` from `indices.min/max`, (3) decode only that window,
    (4) re-base `indices` to the decoded window and fancy-index. Memory usage is bounded
    by the indices span, not the clip length.

    Future swap target: `torchcodec.decoders.VideoDecoder` (when `read_video` is
    ...
    Returns:
        uint8 tensor of shape `[len(indices), 3, H, W]`. Discards audio + info.
    ...
    # Cheap probe for fps + frame count — no frames decoded.
    pts_list, video_fps = torchvision.io.read_video_timestamps(path, pts_unit="sec")
    fps = float(video_fps) if video_fps else 30.0  # Squat is uniformly 30fps; defensive default.

    idx_min = int(indices.min().item())
    idx_max = int(indices.max().item())

    # Window-bounded decode — `+1/fps` on end gives the decoder one extra frame of
    # slack on the right edge to handle floating-point boundary.
    start_pts = idx_min / fps
    end_pts = (idx_max + 1) / fps

    # `output_format="TCHW"` is required — default is "THWC" which would force
    ...
    # Re-base indices to the decoded window.
    ...
    # Hard shape guard — wrong shape here would silently corrupt downstream batches.
...
def spatial_train(
    ...
    *,
    ...
    Args:
        ...
        crop_size:    output spatial size (default 112 — R(2+1)D-18 Kinetics-V1 native).
        ...
        generator:    torch.Generator for the random crop offset (determinism).
...
if __name__ == "__main__":
    ...
    # The window-bounded-decode memory check (on a ≥400-frame .mp4 with peak memory
    # < 200 MB) runs in the notebook, where real /content/squat_videos/*.mp4 are available.
