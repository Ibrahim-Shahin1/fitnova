...
# ──────────────────────────────────────────────────────────────────────────────
# Half-cycle splitter — shared with squat_ssl.py.
# The same argMIN sign (bottom_is_argmax=False) applies to OHP: overhead = barbell
# at top of image = lowest y pixel = argMIN.
# ──────────────────────────────────────────────────────────────────────────────
...
def split_half_cycles(
    ...
    *,
    ...
    Args:
        ...
        frames_per_half: frames to sample from each half-cycle (16).
        ...
        bottom_is_argmax: True → bottom of rep = argmax of smoothed y; False →
            argmin. The SIGN is resolved empirically on real clips because Fig.3
            plots amplitude (possibly inverted) and image-y grows downward — it is
            a PARAM here, never hard-coded.
    ...
    # NOTE: multi-rep find_peaks handling is not implemented here — the paper is nominally
    # single-rep, so argmax/argmin covers the bulk of the 5,490 trajectories.
...
class OHPSSLDataset(Dataset):
    ...
    def _load_trajectory(self, clip_id: str) -> np.ndarray:
        ...
        horizontal bbox covering full image width). y_center = (y1+y2)/2 per ReadMe
        formula.
        ...
        Defensive guard: access frame[0] if len(frame)>0 else [] — in case a file has
        ...
        Returns:
            ...
            1:1 with video frames (assumed, confirmed against real clips).
        ...
        for i, frame in enumerate(raw):
            # Defensive guard: access region_0 = frame[0] if frame has >=1 region.
            ...
            if region_0:
                ...
                # interpolated below. (~40% of files have some empty frames.)
                y[i] = np.nan
    ...
    def _augment(self, clip_u8: torch.Tensor) -> torch.Tensor:
        ...
        views — preventing trivial-positive contrastive collapse.
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        #    negative = an augmented view of the ASCENT (paper §3.2).
        ...
        #    motion is identical across all three. Coin flip on default RNG.
