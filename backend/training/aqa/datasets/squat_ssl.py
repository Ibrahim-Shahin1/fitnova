...
barbell trajectories. Per the paper (§3.2): the anchor is the descent
...
The only genuinely new sampling logic is ``split_half_cycles``;
everything else reuses ``decode_clip`` / ``uniform_sample_indices`` /
``spatial_train`` from ``transforms.py`` and the augmentation primitives in
``ssl_augs.py``.
"""
    ...
    *,
    ...
    Args:
        ...
        bottom_is_argmax: True → bottom of rep = argmax of smoothed y; False →
            argmin. The SIGN is resolved empirically (§1 pt.3) because Fig.3 plots
            amplitude (possibly inverted) and image-y grows downward — it is a PARAM
    ...
    # 1. Smooth to suppress YOLO-detection jitter before extremum detection.
    sm = scipy.ndimage.gaussian_filter1d(traj_y, sigma=smooth_sigma)
    # 2. Bottom-of-rep = the global extremum of the smoothed y-curve.
    ...
    #    empirically by human-verifying on real clips.
    ...
    #    uniform_sample_indices (no new sampler). The bottom frame is the shared
    ...
    # NOTE: multi-rep find_peaks handling is deferred until the multi-rep frequency across
    # the 4,970 trajectories is measured (the paper is nominally single-rep, so argmax/argmin
    # covers the bulk).
...
class SquatSSLDataset(Dataset):
    ...
    def _load_trajectory(self, clip_id: str) -> np.ndarray:
        """Load the barbell-y trajectory for ``clip_id`` (per-clip JSON flat float list).

        The trajectory is 1:1 with video frames (frames/traj == 1.000), so its index space
        IS the video-frame space — no rescaling. In-file NaN/null values (detection gaps,
        e.g. in 25707_3) are linearly interpolated over so ``split_half_cycles``' extremum
        detection is robust.
        """
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        # 1. Split into descent/ascent. bottom_is_argmax=False (ARGMIN) is the empirically
        #    confirmed sign: the rep-bottom is the MIDDLE minimum; argmax lands at the standing
        #    endpoints and degenerates the ascent (argmax ascent collapsed to one frame).
        descent_idx, ascent_idx = split_half_cycles(
            traj_y, frames_per_half=self.frames_per_half, bottom_is_argmax=False,
        )
        # 2. Decode the two half-cycles (1:1 traj->frame). decode_clip -> [16,3,H,W] uint8.
        ...
        #    augmented view of the ASCENT (paper §3.2).
        ...
        #    Coin flip on the default RNG (harness capture/restore covers it).
        if float(torch.rand(1).item()) < 0.5:
            anchor_u8 = anchor_u8.flip(0)
            positive_u8 = positive_u8.flip(0)
        else:
            negative_u8 = negative_u8.flip(0)
        # 5. Spatial pipeline -> [3,16,112,112] float32 Kinetics-normalized (transforms.py).
        return {
...
def build_ssl_loader(dataset: SquatSSLDataset, config: "MDConfig", *, seed: int = 42) -> DataLoader:
    ...
    g = torch.Generator()
    g.manual_seed(seed)
    _persistent = config.num_workers > 0  # avoids PyTorch worker-shutdown stderr spam
    return DataLoader(
        dataset,
