...
Split sizes are 1136/243/244 with full label coverage across all three splits; this module surfaces those records to the dataset class in `squat.py` and to the notebook for reconciliation. Order matches the JSON list in each `{split}_keys.json` for deterministic indexing across runs.
"""
...
# Dataset layout under the Drive root (`My Drive/Fitness-AQA_dataset_release/...`).
...
def _label_value(label_dict: dict, clip_id: str) -> int:
    ...
    Defensive: per the Fitness-AQA schema, every split clip_id must appear in BOTH
    label dicts. Missing keys ⇒ KeyError rather than silent zero — we'd rather
    surface a dataset-format surprise than guess.
    """
    if clip_id not in label_dict:
        raise KeyError(
            f"clip_id '{clip_id}' missing from label dict (every split id must be labeled)"
        )
...
def index(
    ...
    *,
    ...
    if len(records) != expected:
        logger.warning(
            "split '%s' count drift: got %d records, expected %d",
            split,
            len(records),
            expected,
        )
...
def expected_counts() -> dict[str, int]:
    """Return verified split counts. Used by the notebook for reconciliation."""
    return dict(_EXPECTED_COUNTS)


# ──────────────────────────────────────────────────────────────────────────────
# OHP split constants (verified counts + subdir paths)
# ──────────────────────────────────────────────────────────────────────────────
...
def index_ohp(
    ...
    *,
    ...
    Expected counts: train=1582, val=339, test=339.
    ...
    if len(records) != expected:
        logger.warning(
            "OHP split '%s' count drift: got %d records, expected %d",
...
def ohp_expected_counts() -> dict[str, int]:
    """Return verified OHP split counts. Used by the notebook for reconciliation."""
