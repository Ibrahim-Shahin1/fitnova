...
- `mount_drive`, `stage_squat_videos` — Drive FUSE mount and per-session copy of
  `Squat/Labeled_Dataset/videos.zip` → `/content/squat_videos/`. Idempotent:
  re-running a cell does not re-extract.
- `capture_rng_state`, `restore_rng_state` — full 4-RNG capture (Python `random`,
  NumPy, torch CPU, torch CUDA). Sets `cudnn.deterministic=True`,
  `cudnn.benchmark=False`, `torch.use_deterministic_algorithms(True)`.
- `hash_config`, `atomic_save_checkpoint`, `load_latest_checkpoint`,
  `prune_checkpoints`, `CheckpointConfigMismatchError`. Atomic write contract:
  `torch.save` to tmp → reload-verify → `os.replace` → write `latest.txt` LAST. **Drive
  FUSE rename is NOT atomic** — the contract defends against partial checkpoints. The
  `latest.txt`-written-last ordering means a crash mid-write leaves the prior good
  file pointer untouched.
"""
...
# read_video is deprecated; the swap to TorchCodec runs when Colab's torchvision
# passes 0.24 (`read_video` removal). Until then the warning is noise — it fires
# once per `read_video_timestamps` + `read_video` call, polluting stdout during
# the tiny training run.
...
# By the time this module loads, _envinit.py has already set CUBLAS_WORKSPACE_CONFIG.
# So `torch.use_deterministic_algorithms(True)` will not raise because of missing
# workspace config. `warn_only=True` keeps any genuinely non-deterministic op from
# crashing the run — they'll log a warning instead.
torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False
torch.use_deterministic_algorithms(True, warn_only=True)

# videos.zip ships 1739 mp4s for the Squat labeled set (116 extra beyond the 1623
# official-split ids, intentionally excluded by the split JSONs).
...
def _extract_with_resume_and_progress(zip_path: Path, dest: Path) -> float:
    ...
    `videos/12345.mp4`; `splits.py` builds `video_path = videos_root + "/" + clip_id +
...
def stage_squat_videos(
    ...
    *,
    ...
    """Copy Squat `videos.zip` from Drive → `/content/`, extract, verify count. Idempotent with **byte-level resume**.

    Drive FUSE per-frame reads are too slow for training (~2.5s per archive open from
    Drive vs. effectively zero from local disk). Per-session staging to `/content/`
    local disk is the build constraint for any video iteration.
    ...
    **Three-layer resume design (per the working-agreement disconnect-by-default rule):**
    ...
    Args:
        ...
        expect_count: expected mp4 count (1739 for Squat labeled).
    ...
    # Idempotence — three checks before falling through to copy + extract:
    ...
    # the TOP of local_root (the splits.py contract).
...
_SQUAT_UNLABELED_VIDEOS_EXPECT_COUNT = 4970  # Squat Unlabeled_Dataset mp4 count
...
def stage_unlabeled_squat_videos(
    ...
    *,
    ...
    """Stage the unlabeled Squat `videos.zip` + `bar_trajectories_raw.zip` from Drive.
    ...
    Args:
        ...
        expect_count:      expected mp4 count (4970 unlabeled Squat clips).
...
# Labeled zip holds 2,367 mp4s (2,260 official + 107 extra — like Squat's
# 1,739-vs-1,623; the split JSONs are the id source of truth).
_OHP_LABELED_VIDEOS_EXPECT_COUNT   = 2367
# 5,490 unlabeled OHP clips.
...
def stage_ohp_videos(
    ...
    *,
    ...
    Args:
        ...
        expect_count: expected mp4 count (2367 for OHP labeled zip).
...
def stage_unlabeled_ohp_videos(
    ...
    *,
    ...
    OHP archive is SPLIT across two release folders:
    ...
    (mp4-only): always use zipfile.extractall for JSON trajectory zips.
    ...
    Args:
        ...
        expect_count:      expected mp4 count (5490 unlabeled OHP clips).
    ...
    # NOT _extract_with_resume_and_progress (mp4-only).
...
# ──────────────────────────────────────────────────────────────────────────────
# Shallow-Squat image staging + CVCSPC SSL frame extraction
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# RNG capture/restore. Required for the bitwise-resume guarantee (fresh-2-epoch vs.
# resumed-from-epoch-0-into-epoch-1 must produce byte-identical epoch-1 loss
# trajectories). All 4 RNG sources captured so seed-restoration is exhaustive: any
# one missing would let post-resume sampling drift.
# ──────────────────────────────────────────────────────────────────────────────
...
def capture_rng_state() -> dict:
    ...
    The 4-key contract is a determinism precondition: every source must be captured
    for resume to be bitwise-exact.
    """
    return {
...
def restore_rng_state(state: dict) -> None:
    ...
    Order: **CUDA first, then CPU** to avoid drift — by setting CUDA before any CPU
    op that might internally allocate or seed a CUDA tensor, we keep CUDA's seed
    exactly at the captured value. The CPU/numpy/python restores can then happen in
    any order since they're independent streams.
...
# ──────────────────────────────────────────────────────────────────────────────
# Atomic checkpoint primitives. Drive FUSE rename is NOT atomic; these primitives
# defend against partial writes with:
...
def hash_config(config: dict) -> str:
    """SHA-256 of `json.dumps(config, sort_keys=True, default=str)`, truncated to 16 hex chars.
...
def atomic_save_checkpoint(
    ...
    """Atomically save `payload` to `target_path`; update `latest.txt` iff `update_latest`.

    `update_latest` (default True) preserves the default epoch-pointer behavior. Pass
    `update_latest=False` for `backbone.pt` / `best.pt` writes so the `latest.txt`
    epoch-resume pointer is never clobbered — otherwise a `backbone.pt` save would make
    SSL resume jump to the wrong (best, not latest) epoch.
    ...
    if update_latest:
        ...
        # backbone.pt / best.pt write — leave the epoch-resume pointer alone.
...
def prune_checkpoints(
    ...
    *,
    ...
    """Keep the `keep_last` most-recent `epoch_*.pt` (by mtime) + optionally `best.pt`; delete the rest.
