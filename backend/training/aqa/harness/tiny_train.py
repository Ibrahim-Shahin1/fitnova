...
public entrypoint. With `max_epochs >= 2` and `resume=True`, the Task 14 bitwise
...
def run_tiny_epoch(
    *,
    ...
    Args:
        ...
        max_train_clips: Phase 2 tiny — clamp train Subset to this many clips.
        max_val_clips:   Same for val.
        batch_size:      DataLoader batch size.
        crop_size:       Spatial crop (D1 default 112).
    ...
    config = {
        ...
        "loss": "BCEWithLogitsLoss(no pos_weight)",  # Phase 2 tiny: no pos_weight per D9
    }
    ...
    # Build full datasets then Subset-wrap to clamp wall time. Phase 2 tiny.
    ...
    # Phase 2 tiny: Subset preserves the rest of the pipeline (decode, sampler,
    # transforms) but caps the number of real videos touched per epoch.
    ...
    # Phase 2 tiny: num_workers=0 (D14 bitwise-determinism precondition 5).
    ...
    loss_fn = nn.BCEWithLogitsLoss()  # Phase 2 tiny: no pos_weight (D9)
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        # D10 + F6 checkpoint payload schema
        payload = {
            "epoch": epoch,
