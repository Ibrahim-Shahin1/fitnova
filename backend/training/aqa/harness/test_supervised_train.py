...
Covers SQUAT-03-a (model head shape) and SQUAT-03-e (checkpoint schema keys)
per the 03-VALIDATION.md per-task verification map.
...
See: .planning/phases/03-squat-supervised-baseline/03-CONTEXT.md — D1 / D8.
"""
...
# Top-level Phase 3 D8 payload keys that must appear on every checkpoint dict.
...
# Per-epoch metrics_history entry keys (Phase 3 D8 — extends Phase 2 schema).
...
# ─────────────────────────────────────────────────────────────────────────────
# SQUAT-03-e: checkpoint schema keys (pure-Python schema regression — fast)
# ─────────────────────────────────────────────────────────────────────────────


def test_checkpoint_schema_keys() -> None:
    """Phase 3 D8 schema regression — every key the trainer writes must be present.

    Build a dummy payload mirroring what `run_supervised_epoch` produces. Assert
    every Phase 3 top-level key and every Phase 3 metrics_history sub-key is
    present, and assert `code_version == "phase03-supervised-baseline"` so a
    Phase 2 checkpoint can never silently pass as a Phase 3 checkpoint.
    """
    dummy_metrics_entry = {
        "epoch": 0,
        "train_loss_mean": 0.0,
    ...
    assert payload["code_version"] == "phase03-supervised-baseline", (
        ...
        f"{payload['code_version']!r}) — Phase 2 checkpoint must NOT pass "
        f"as Phase 3"
    )


# ─────────────────────────────────────────────────────────────────────────────
# SQUAT-03-a: model head shape (slow — downloads ~120 MB weights on first call)
# ─────────────────────────────────────────────────────────────────────────────
