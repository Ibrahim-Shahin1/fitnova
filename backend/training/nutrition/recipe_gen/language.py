...
# FitNova patch (2026-05-29): the upstream `pytorch_pretrained_bert` is archived.
# Switched to the modern HF `transformers` (same OpenAI GPT-1 vocab/IDs). The tokenizer
# is only needed for tokenize_string + decode; constants are hardcoded so the package
# imports even without transformers installed (e.g. for a CPU forward-pass smoke test).
try:
...
# Vocabulary — OpenAI GPT-1 BPE = 40,478 subwords (hardcoded so import works without the tokenizer)
...
# Decoder mapping — populated from the tokenizer when available
DECODER = {v: k for k, v in TOKENIZER.encoder.items()} if TOKENIZER is not None else {}
DECODER.update({
    SOS_INDEX: '<s>',
    EOS_INDEX: '</s>',
    PAD_INDEX: '</p>',
...
# FitNova patch (2026-05-29): only built when the tokenizer is available
TECHNIQUE_TOKENS = [
