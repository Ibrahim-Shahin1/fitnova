...
in-the-wild; MediaPipe was removed from the serving path.  Rep segmentation therefore
operates on motion energy (frame-difference signal) — no pose, no optical flow.
...
Live mode (sliding-window trigger):
  ...
  WebSocket / JPEG decode layer (SquatLiveSession wires this in).

Why frame-difference over optical flow:
...
def segment_reps_by_motion_energy(
    ...
    FALLBACK (ALWAYS returns >= 1 interval):
      ...
      Fitness-AQA single-rep training clips.
...
class LiveWindowTrigger:
    """Per-connection inference trigger for the live WebSocket path.
    ...
    a running WebSocket connection.  SquatLiveSession calls should_fire()
