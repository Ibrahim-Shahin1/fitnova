...
"""
    ...
    def generate_plan(
        self, program_id: int, user_profile: dict, candidates=None
    ) -> dict:
        """Generate the plan via the CrewAI 4-agent pipeline (Profiler →
        Generator → Critic → Optimizer), grounded ONLY in the recommended
        program's exercise pool. No template fallback: a crew failure propagates
        (surfaced as a 500 / coach tool error). `candidates` is the recommender's
        content_candidates, used to augment the pool for under-represented
        movement groups without inventing exercises.
        """
        from backend.services import plan_crew

        program = self._get_program(program_id)
        return plan_crew.build_plan(self, program, user_profile, candidates)
