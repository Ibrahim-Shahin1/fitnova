...
D-11 DOMAIN-SHIFT FIX — `kneeaware_spatial_val`:
  The real-camera domain-shift test (D-11) revealed that on PORTRAIT phone video (e.g.
...
upload endpoint `POST /analyze-form-video` (Plan 05-03), and `SquatFormService.classify_clip`.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# D-11 knee-aware spatial preprocessing constant
# ─────────────────────────────────────────────────────────────────────────────
...
# 112² crop.  Value tuned on 4 real phone clips (D-11 validation run, 2026-05).
...
def kneeaware_spatial_val(
    ...
    *,
    ...
    WHY (D-11 domain-shift fix, 2026-05):
      ...
      bad≈0.13–0.24 (directionally correct).  User-approved 2026-05-26.
