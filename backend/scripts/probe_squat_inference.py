...
Addresses:
  - D-08: measure per-clip CPU forward latency (single-seed vs 3-seed ensemble)
    on the target machine with REAL staged weights.
  - D-11: run the model end-to-end on a REAL user phone-camera squat clip and
...
Output goes to stdout - paste back the full output at the checkpoint.

IMPORTANT: fp32 only throughout - do NOT add .half() calls.
fp16 hangs on CPU with PyTorch 2.12 on Windows (Pitfall 4, RESEARCH Common Pitfalls).
"""
...
def main() -> None:
    ...
    # -- 1. Load service ---------------------------------------------------------
    print("=" * 70)
    print("SQUAT INFERENCE PROBE - D-08 (latency) + D-11 (domain-shift)")
    print("=" * 70)
    ...
    print()
    print("-" * 70)
    print("SECTION A - CPU LATENCY (D-08)")
    print("-" * 70)
    ...
    # Compare against RESEARCH section 4 baseline
    ref_single = 0.93
    ref_ensemble = 2.8
    ...
    print(f"  vs RESEARCH s4 baseline: single {sign_s}{delta_single:.3f} s, ensemble {sign_e}{delta_ensemble:.3f} s")

    # -- 3. REAL CLIP (D-11) -----------------------------------------------------
    if clip_path is None:
        print()
        print("-" * 70)
        print("SECTION B - REAL-CLIP DOMAIN-SHIFT TEST (D-11)")
        print("-" * 70)
    ...
    print()
    print("-" * 70)
    print("REAL-CLIP RESULTS (D-05 response schema)")
    print("-" * 70)
    ...
    print()
    print("FULL D-05 RESPONSE (via classify_clip - sanity check)")
    import json
    ...
    print("-" * 70)
    print("Expected behavior on a real squat clip (Phase-4 SUMMARY):")
