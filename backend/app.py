...
from backend.services.nutrition_meal_crew import NutritionMealCrew
...
class ErrorDetection(BaseModel):
    type: str = Field(..., pattern=r"^(KIE|KFE|ELBOWS|KNEES|DEPTH)$")
    detected: bool
    confidence: float = Field(..., ge=0.0, le=1.0)
    threshold: float = Field(..., ge=0.0, le=1.0)
    ground_truth: int | None = None  # None for arbitrary upload; set from the catalog when a clip_id is supplied
...
# ─────────────────────────────────────────────────────────────────────────────
# Live WebSocket session — SquatLiveSession
# ─────────────────────────────────────────────────────────────────────────────

# Deterministic feedback templates (no LLM).
_ERROR_FEEDBACK = {
    "KIE": {
...
class SquatLiveSession:
    """Per-WebSocket-connection session for live squat form analysis.
    ...
    single-seed (low-latency), and accumulates per-rep results.
    ...
    Model is read-only — no shared mutable inference state.

    Buffer cap: deque(maxlen=LIVE_BUFFER_MAX_FRAMES=90) — evicts old frames,
    caps per-session memory at ~3 s of frames.
    """

    def __init__(self, service) -> None:
        self._service = service
        # Raw RGB np.ndarray frames from cv2; LIVE_BUFFER_MAX_FRAMES caps memory.
    ...
    def push_frame(self, jpeg_bytes: bytes) -> bool:
        ...
        Security: a malformed JPEG (cv2.imdecode -> None) is dropped and
    ...
    def end_session(self) -> dict:
        """Finalize the session and return a deterministic session_summary dict.

        session_feedback is assembled from per-rep KIE/KFE detections using
        _ERROR_FEEDBACK templates — no LLM call.

        Returns:
            Dict matching the SessionSummary shape:
    ...
    def _build_feedback(self) -> str:
        """Build deterministic session feedback from per-rep detections (no LLM)."""
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Verifying CrewAI plan environment (Layer 3 — 4-agent crew)...")
    try:
        from backend.services.crew_env import ensure_crew_env
        _crew = ensure_crew_env()
        if _crew["ok"]:
            logger.info("CrewAI plan env ready: crewai %s (rebuilt=%s)",
                        _crew["version"], _crew["rebuilt"])
        else:
            logger.error(
                "CrewAI plan env NOT ready: %s\n"
                "Plan generation will fail until the venv is fixed. Rebuild manually:\n"
                "  python -m venv backend/.crewenv\n"
                "  backend/.crewenv/Scripts/python -m pip install -r backend/requirements-crew.txt",
                _crew["error"])
    except Exception:
        logger.exception("CrewAI plan env verification crashed")
    logger.info("Loading chat service...")
    app.state.chat_service = ChatService()
    logger.info("Loading coach service...")
    app.state.coach_service = CoachChatService()
    logger.info("Loading nutrition service (BMI/calorie engine + recipe selector + 4-agent crew)...")
    try:
        _oa_client = None
        try:
            ...
            if _key:
                _oa_client = OpenAI(api_key=_key)
        except Exception:
            _oa_client = None
        app.state.nutrition_crew = NutritionMealCrew(
            client=_oa_client, planner=MealPlanner(NutritionRecipeSelector()))
        logger.info("Nutrition crew loaded (llm=%s)", _oa_client is not None)
    except Exception as exc:
        logger.warning("Nutrition service failed to load: %s", exc)
        app.state.nutrition_crew = None
    try:
        from backend.services.recipe_generator import RecipeGenerator
        app.state.recipe_generator = RecipeGenerator()
        logger.info("Recipe generator: available=%s", app.state.recipe_generator.available)
    except Exception as exc:
        logger.warning("Recipe generator failed to init: %s", exc)
        app.state.recipe_generator = None
    ...
    logger.info("Loading OHP form service (PyTorch R(2+1)D-18, 2-seed)...")
    from backend.services.ohp_form_service import OHPFormService
    _ohp_dir = os.path.join(os.path.dirname(__file__), "models", "form_model_ohp_md")
    app.state.ohp_form_service = OHPFormService(model_dir=_ohp_dir)
    logger.info(
        "OHPFormService ready: model_ready=%s, seeds_loaded=%d",
        app.state.ohp_form_service.model_ready,
        len(app.state.ohp_form_service._models),
    )
    logger.info("Loading Shallow-Squat form service (PyTorch ResNet-18, 3-seed)...")
    from backend.services.shallow_squat_form_service import ShallowSquatFormService
    _shallow_dir = os.path.join(os.path.dirname(__file__), "models", "form_model_shallow_cvcspc")
    app.state.shallow_form_service = ShallowSquatFormService(model_dir=_shallow_dir)
    logger.info(
        "ShallowSquatFormService ready: model_ready=%s, seeds_loaded=%d",
        app.state.shallow_form_service.model_ready,
        len(app.state.shallow_form_service._models),
    )
    logger.info("Loading benchmark catalog...")
    from backend.services.benchmark_catalog import BenchmarkCatalog
    _aqa_root = "Fitness-AQA/Fitness-AQA_dataset_release-20260518T073812Z-3-001/Fitness-AQA_dataset_release"
    app.state.benchmark_catalog = BenchmarkCatalog(
        fitness_aqa_root=_aqa_root,
...
async def form_session_ws(websocket: WebSocket):
    """Real-time squat form analysis via WebSocket.
    ...
    accepted WebSocket — no shared mutable state.
    Malformed JPEG frames are silently dropped; the session continues.
    """
    ...
    try:
        while True:
            ...
            if msg_type == "start_session":
                # Construct a fresh per-connection session (no shared mutable state).
...
# Reject uploads > 100 MB before buffering the body into memory.
...
_SVC_ATTR: dict[str, str] = {
    "squat": "squat_form_service",
    "ohp":   "ohp_form_service",
    "shallow": "shallow_form_service",
}
...
async def analyze_form_video(
    ...
    clip_id: str | None = Form(default=None),
):
    """
    Analyze a pre-recorded exercise clip for form errors.

    For squat/ohp: upload a video file; the clip is treated as one rep and
    classified through the PyTorch R(2+1)D-18 ensemble.
    For shallow: supply a catalog clip_id; the JPEG crop is classified directly
    via the ResNet-18 image classifier (no video decode).

    When clip_id is provided for any exercise, ground_truth labels are attached
    to each error from the BenchmarkCatalog.
    """
    import tempfile

    if exercise not in {"squat", "ohp", "shallow"}:
        raise HTTPException(status_code=400, detail="exercise must be one of: squat, ohp, shallow")

    svc = getattr(request.app.state, _SVC_ATTR[exercise])
    catalog = request.app.state.benchmark_catalog

    # ── Shallow: image-only path (no video decode) ────────────────────────────
    if exercise == "shallow":
        if clip_id is None:
            raise HTTPException(status_code=400, detail="clip_id required for shallow exercise")
        if clip_id not in catalog.whitelisted_ids.get("shallow", set()):
            raise HTTPException(status_code=404, detail="clip_id not in test split")
        image_path = catalog.media_path("shallow", clip_id)
        try:
            result = await svc.classify_image_async(str(image_path))

            # Build thumbnail from the crop itself — it IS the analyzed image.
            img_bgr = cv2.imread(str(image_path))
            if img_bgr is not None:
                ok, buf = cv2.imencode(".jpg", img_bgr, [cv2.IMWRITE_JPEG_QUALITY, 75])
                thumbnail_b64 = base64.b64encode(buf.tobytes()).decode() if ok else None
            else:
                thumbnail_b64 = None

            clip_gt: dict[str, int] = {}
            if clip_id:
                gt_entry = next(
                    (e for e in catalog.get_catalog("shallow") if e["clip_id"] == clip_id), None
                )
                if gt_entry:
                    clip_gt = gt_entry["ground_truth"]
            ...
            for err in result.get("errors", []):
                errors.append(ErrorDetection(
                    ...
                    ground_truth=clip_gt.get(err["type"]),
                    intervals=err.get("intervals", []),
                ))
        except HTTPException:
            raise
        except Exception as exc:
            logger.exception("Shallow form analysis inference failed")
    ...
    # ── Squat / OHP: video path ───────────────────────────────────────────────
    if file is None:
        raise HTTPException(status_code=400, detail="file required for squat/ohp exercise")

    # Content-length pre-check (fast-path; defends well-behaved clients).
    ...
    # tmp_path None-guard so finally: never NameErrors on early exit.
    tmp_path: str | None = None
    try:
        with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as tmp:
            tmp_path = tmp.name
            total_read = 0
            # Chunked read cap — defends missing/forged content-length.
            while True:
        ...
        try:
            ...
            clip_gt_video: dict[str, int] = {}
            if clip_id:
                gt_entry = next(
                    (e for e in catalog.get_catalog(exercise) if e["clip_id"] == clip_id), None
                )
                if gt_entry:
                    clip_gt_video = gt_entry["ground_truth"]

            errors_video: list[ErrorDetection] = []
            for err in result.get("errors", []):
                intervals = [[0.0, round(duration_s, 2)]] if err.get("detected") else []
                errors_video.append(ErrorDetection(
                    type=err["type"],
                    detected=err["detected"],
                    confidence=err["confidence"],
                    threshold=err["threshold"],
                    ground_truth=clip_gt_video.get(err["type"]),
                    intervals=intervals,
                ))

            # Mid-frame thumbnail: representative frame the model analyzed, as base64 JPEG.
            frames_np = frames.numpy()
            mid_idx = len(idx) // 2
            frame_hwc = frames_np[mid_idx].transpose(1, 2, 0)   # [H,W,3] RGB
            frame_bgr = frame_hwc[:, :, ::-1]
            ok, buf = cv2.imencode(".jpg", frame_bgr, [cv2.IMWRITE_JPEG_QUALITY, 75])
            thumbnail_b64 = base64.b64encode(buf.tobytes()).decode() if ok else None

            model_view = ModelView(frames=_model_view_frames(frames_np))
        except HTTPException:
            raise
        except Exception as exc:
        ...
        # Clean up temp file; guard against None if NamedTemporaryFile raised.
        if tmp_path:
            try:
                os.unlink(tmp_path)
            except OSError:
...
# ─────────────────────────────────────────────────────────────────────────────
# Benchmark catalog + media endpoints
# ─────────────────────────────────────────────────────────────────────────────


@app.get("/api/benchmark/catalog", tags=["Benchmark"])
async def get_benchmark_catalog(request: Request, exercise: str):
    """Return the full test-split catalog for `exercise` (squat/ohp/shallow).

    Each entry carries clip_id, ground_truth per error, and the results.pkl score.
    """
    if exercise not in {"squat", "ohp", "shallow"}:
        raise HTTPException(status_code=400, detail="exercise must be one of: squat, ohp, shallow")
    catalog = request.app.state.benchmark_catalog
    return {"exercise": exercise, "clips": catalog.get_catalog(exercise)}


@app.get("/api/benchmark/media/{exercise}/{clip_id}", tags=["Benchmark"])
async def serve_benchmark_media(request: Request, exercise: str, clip_id: str):
    """Stream the media file for a whitelisted test-split clip.

    The clip_id whitelist check happens BEFORE any filesystem path is constructed —
    this is the path-traversal gate.
    """
    if exercise not in {"squat", "ohp", "shallow"}:
        raise HTTPException(status_code=400, detail="Unknown exercise")
    catalog = request.app.state.benchmark_catalog
    if clip_id not in catalog.whitelisted_ids.get(exercise, set()):
        raise HTTPException(status_code=404, detail="clip_id not in test split")
    path = catalog.media_path(exercise, clip_id)
    if not path.is_file():
        raise HTTPException(status_code=404, detail="media not found for clip_id")
